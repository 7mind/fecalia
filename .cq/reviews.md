...
- createdAt: 2026-07-15T06:50:52.111Z
- updatedAt: 2026-07-15T06:50:52.111Z
- author: "opus-4.8[1m]"
- session: 671d5adc-7e2a-440e-b87d-6da40edeb7b7
- summary: "G19 IMPLEMENT review (orchestrator inline, proportionate test-edit-only) — VERDICT = go-ahead. Merged to main as a26e6ae (T187/T192; test/e2e/multipeer_test.go + restart_onesided_test.go + netns.go). T187 (D81): the non-primary peer INBOUND attribution assertion in multipeer_test.go's independent-inner-streams subtest switched from MetricRxBytes (structurally 0 for a non-primary peer on a shared Bind readLoop) to MetricTxBytes (which IS per-peer), mirroring the file's own corrected sibling + multipeer_hardened_test.go testD47SharedNATDemux; stale comment corrected. T192 (D80): restart_onesided_test.go's concentrator /metrics rebound from the non-loopback uplink IP (which requireLoopback refuses) to 127.0.0.1:9104 INSIDE the peer netns, scraped via the proven D77 fetchMetricsInNetns remediation; the worker correctly discovered the actual scrape sites (the task's line numbers were half-right — lines 228/254 scrape the edge's own base-netns loopback and correctly stay direct) and added a needed waitSessionEstablishedInNetns helper mirroring hwFixture.waitPeerPathUp; netns.go port-registry comment updated. GATE: go vet -tags e2e + go build -tags e2e + golangci-lint --build-tags e2e ./test/e2e/... all clean; full just build/test/lint green. SCOPE NOTE: these are -tags e2e privileged (netns/TUN/CAP_NET_ADMIN) tests — the fixes are deterministic code edits validated at the tagless (vet/build/lint) level; full functional validation (the tests actually running to completion) is DEFERRED to the o3.7mind.io / llm-ubuntu-0.pgtr.7mind.io hardware tier, consistent with the e2e suite's standing hardware-deferral. 0 criticisms. On this merge D80 + D81 -> resolved (with the privileged-run validation tracked as the standing e2e hardware deferral)."
- ledgerRefs: ["goals:G19","tasks:T187","tasks:T192","defects:D80","defects:D81"]
...
- createdAt: 2026-07-15T06:50:15.740Z
- updatedAt: 2026-07-15T06:50:15.740Z
- author: "opus-4.8[1m]"
- session: 671d5adc-7e2a-440e-b87d-6da40edeb7b7
...
- createdAt: 2026-07-20T18:22:15.776Z
- updatedAt: 2026-07-20T18:22:15.776Z
- author: "opus-4.8[1m]"
- session: 671d5adc-7e2a-440e-b87d-6da40edeb7b7
- summary: "G23 plan review round 1 — RECONCILED REVISE (opus + fable configured panel, strictest-wins; BOTH revise). Core plan verified SOUND by both: well-grounded, reproduce-first, honors every load-bearing invariant (config import-cycle mirror, complete-100B overhead / no double-subtraction, D16 3s-budget hoist into telemetry with thresholds.go re-alias, RideThrough=0 byte-identical, disjoint OUTPUT-vs-FORWARD MSS chains); DAG acyclic + correctly sequenced. Five planner-fixable findings drive REVISE round 2 (aggregated + deduped across both reviewers)."
- criticism: ["[BOTH] T210 (and by extension T212) netns e2e negative-control is not reproducible in a lossless netns: without DF, a 1500B outer datagram on a 1400-MTU veth is LOCALLY FRAGMENTED and reassembles with ~0 loss, so the 'loss>50%' assert is green-by-fragmentation and would not fail for the right reason (field loss came from the 5G network DROPPING fragments). FIX: T210 must contain its own loss mechanism within M82 scope — an nftables/tc rule in the transit netns dropping IP fragments (or oversize datagrams), mirroring the field middlebox — and phase-2's 'no MTU-induced loss' must run against that same mechanism. Same fix applies to T212.","[opus] T211 aliasing test/e2e/thresholds.go P1RecoverySeconds (currently an untyped int = 3, a count of SECONDS) to the new telemetry.RecoveryBudget (a time.Duration = 3*time.Second) silently changes its TYPE and UNIT and breaks call sites using it as int-seconds (e.g. time.Duration(P1RecoverySeconds)*time.Second). FIX: keep P1RecoverySeconds an int-seconds constant and add telemetry.RecoveryBudget as a SEPARATE Duration (thresholds.go re-derives from it), OR migrate every call site to Duration comparison — T211 must state which.","[opus] T201 puts the Linux-only IP_MTU_DISCOVER/IPV6_MTU_DISCOVER setsockopt in the untagged internal/bind/pathsock.go, which would break the non-Linux build (the analogous SO_BINDTODEVICE lives in pathsock_linux.go behind //go:build linux). FIX: place the DF setsockopt behind the same platform split (a linux impl + a portable no-op/stub), mirroring bindToDevice.","[BOTH] D85 FIX-DIRECTION 4 (subtract the amnezia junk-PREFIX headroom when obfuscation is enabled — the confirmed obfuscation-conditional field-excess per mtu.go:41-46) has NO task and no de-scope. WORSE: the T202/T206 padded-probe PMTU discovery measures probe-plane datagrams that do NOT carry the junk prefix real WG DATA carries, so on an obfuscated path discovery over-estimates usable size and re-introduces the field failure. FIX: this is IN SCOPE (it is in D85's accepted suggestedFix) — add a task to subtract the configured junk-prefix headroom in tunMTU/InnerMTU when obfuscation is enabled AND pad discovery probes by that headroom (so discovery measures the true data-frame envelope). Do NOT ask the user — it is already-accepted fix scope.","[fable] T203 validates only down_after>0, but probe cadence stays fixed at 200ms this pass: a down_after at/below one probe interval makes Tick's strict-'>' silence check outrun the echo cadence and permanently flap every path DOWN — a foot-gun the T211 upper-side WARN-and-allow budget does not cover. FIX: add a validate() lower bound (reject or WARN on down_after < 2*DefaultProbeInterval), choice documented."]
- new_questions: []
- ledgerRefs: ["goals:G23","defects:D85","defects:D86"]

### R243 — go-ahead

- createdAt: 2026-07-20T18:30:38.236Z
- updatedAt: 2026-07-20T18:30:38.236Z
- author: "opus-4.8[1m]"
- session: 671d5adc-7e2a-440e-b87d-6da40edeb7b7
- summary: "G23 plan review round 2 — GO-AHEAD (opus verification reviewer, proportionate: round-1 was dual opus+fable, round-2 verifies the applied fixes). All FIVE R241 findings verified genuinely resolved against the current ledger + source: (1) T210+T212 both install a transit-netns IP-fragment-DROP rule so the negative-control is actually lossy (green-by-fragmentation closed); (2) T211 keeps P1RecoverySeconds an int-seconds constant (=3, thresholds.go:19) + a SEPARATE telemetry.RecoveryBudget Duration with a RecoveryBudget==P1RecoverySeconds*Second identity assertion (no type/unit break); (3) T201 places IP_MTU_DISCOVER behind //go:build linux (pathsock_linux.go) + portable stub (pathsock.go confirmed untagged); (4) NEW T225 (deps T205,T206) closes D85 fix-direction 4 — static junk-prefix subtraction + dynamic probe-padding when obfuscation enabled; (5) T203 adds the down_after>=2*DefaultProbeInterval (>=400ms) floor. DAG acyclic with T225 added. 0 criticisms / 0 questions / 0 defects. G23 plan APPROVED (15 tasks, M82/M83/M84)."
- ledgerRefs: ["goals:G23","defects:D85","defects:D86"]
...
- createdAt: 2026-07-20T18:22:38.089Z
- updatedAt: 2026-07-20T18:22:38.089Z
- author: "opus-4.8[1m]"
- session: 671d5adc-7e2a-440e-b87d-6da40edeb7b7
- summary: "G21 plan review round 1 — RECONCILED REVISE (opus + fable configured panel, strictest-wins; BOTH revise). Core plan verified SOUND by both: 4-milestone contract->plumbing->frontend->docs/gate spine covers accepted Q60-Q65 scope; the loopback redaction gate is genuinely SERVER-SIDE (marshaled-bytes + e2e raw-frame scans, kernel-bound verifyLoopbackBind(ln.Addr())); metrics.Source stays untouched (no new series). Q64 'no new mechanism' verified true (dispatchInbound setRemote at multipath.go:2038-2041). Findings drive REVISE round 2 (aggregated + deduped)."
- criticism: ["[BOTH] T214 grows BuildSnapshot's signature to (src, info, revealAddressing) but defers its 4 existing server.go call-site updates (newWSHandler push loop) to T219 — so internal/monitor will NOT compile between T214 and T219 and T214/T215's own build/test gates cannot pass. FIX: T214 must update ALL BuildSnapshot call sites in the SAME task, fail-closed (revealAddressing=false, placeholder/zero Info), so the tree compiles and redacts by default; leave only the kernel-bound verdict + real Info construction to T219/T222.","[opus] Field-ordering inversion: T214/T215 (M85) must populate then redact the per-path addressing fields, but those metrics.PathSnapshot fields are only ADDED in T220 (M86, which runs AFTER M85). FIX: add the metrics.PathSnapshot addressing field DEFINITIONS in T214 (contract task), leaving only the bind->metrics VALUE wiring in T220. (Removes the cross-milestone ordering hazard.)","[opus] Failover-state freshness: if monitor.Info captures endpoints/active ONCE at device.Up construction, the rendered active-hub freezes at startup and never reflects a later failover — silently defeating the headline Q61c feature. FIX: Info must expose endpoints via a LIVE per-snapshot call evaluated inside BuildSnapshot each frame; add a T223 assertion that the active entry MOVES in the /ws frame after a forced failover.","[BOTH] Q63 OVER-DISCLOSURE: Q63's recorded answer is the LITERAL '(b) truncated fingerprint only, on any binding' (the ONE question NOT answered 'as recommended'; option (c) full-key-loopback-only was on offer and NOT chosen). The plan (T214/T215/T221/T223) surfaces the FULL wgPublicKey on loopback — exceeding the literal answer. FIX (resolved, no user ask needed — tighten to match the user's explicit choice): DROP the full wgPublicKey surface entirely; ship the truncated fingerprint ONLY, on any binding. Update T214 (contract: remove wgPublicKey field), T215 (redaction: fingerprint always, no full key to gate), T221 (render fingerprint only), T223 (assert fingerprint present, full key NEVER present on any binding).","[fable] Dual-source ambiguity: bindMode/boundDevice are delivered by BOTH the monitor.Info config seam (T214/T222) AND the PathTraffic->metrics pass-through (T216/T220), with no authority rule; and T222 lists a config-derived 'Device' but config.Path has NO Device field (config.go:529-573 — only Bind; the bound interface is runtime-resolved in pathsock.go). FIX: designate ONE source per wire field — runtime-resolved BindMode/BoundDevice via the PathTraffic pass-through; config-declared linkBandwidth/linkRtt via Info — drop 'Device' from T222's config list and fix T214's mislabel of boundDevice as 'config-static'.","[fable] T219's NewServer call-site list (device.go + server_test.go) is non-exhaustive — monitor_e2e_test.go and monitor_wire_test.go also construct NewServer. FIX: T219 must update ALL monitor.NewServer call sites in the same task.","[fable] Concentrator empty-endpoint rendering unspecified: on the concentrator config.Peer.Endpoints is empty by config rule, so T217's config-fallback yields an empty list. FIX: T221 must state the dashboard OMITS the endpoint section for an empty list (not an empty 'active' row); T223's 'exactly one active entry' assertion is scoped to the edge-side topology it tests.","[opus] T214 defines both boundDevice ('resolved interface name', any binding) and sourceInterface (inside the gated addressing block) with overlapping semantics. FIX: state the distinct semantics (SO_BINDTODEVICE device name vs the source-IP's owning interface) or collapse to one field, so the worker + types.ts mirror (T218) do not diverge."]
- new_questions: []
- ledgerRefs: ["goals:G21"]

### R244 — go-ahead

- createdAt: 2026-07-20T18:30:48.628Z
- updatedAt: 2026-07-20T18:30:48.628Z
- author: "opus-4.8[1m]"
- session: 671d5adc-7e2a-440e-b87d-6da40edeb7b7
- summary: "G21 plan review round 2 — GO-AHEAD after the sole residual fix (opus verification reviewer). It verified 7 of 8 R242 findings genuinely resolved (atomic BuildSnapshot signature + call-site updates; metrics.PathSnapshot field defs moved into T214 with T220 value-wiring-only; LIVE endpoints provider + T223 active-entry-moves-after-failover freshness assertion; bindMode/boundDevice single-authority via PathTraffic + 'Device' dropped from T222; all NewServer call sites in T219; empty-endpoint-section omission in T221; boundDevice-vs-sourceInterface disambiguation) and the DAG acyclic. It caught ONE residual: the Q63 fingerprint-only fix was not propagated to T218 (the types.ts mirror still listed a phantom optional wgPublicKey? that tsc would not flag, reintroducing the forbidden full-key surface). RESOLVED: T218 updated to mirror wgPublicKeyFingerprint ONLY + a test asserting no wgPublicKey property exists. With that single textual field-list fix applied (the reviewer's exact demand; the other 7 findings + DAG already verified), the G21 plan is APPROVED (11 tasks, M85/M86/M87/M88)."
- ledgerRefs: ["goals:G21"]
