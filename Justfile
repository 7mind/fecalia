...
lint:
    ...
    # Explicit package list (not bare `golangci-lint run`, which walks the repo
    # root): keeps the walk hermetic against .claude/worktrees/ sibling-agent
    # checkouts living beside this tree (D54) — every tracked Go package lives
    # under cmd/, internal/, test/.
