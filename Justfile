...
lint:
    ...
    # Explicit package list (every tracked Go package lives under cmd/,
    # internal/, test/) rather than a bare recursive `golangci-lint run`,
    # keeping the run reproducible and scoped to tracked code (D54/D61).
    golangci-lint run --max-same-issues 0 --max-issues-per-linter 0 ./cmd/... ./internal/... ./test/...
    golangci-lint run --max-same-issues 0 --max-issues-per-linter 0 --build-tags e2e ./test/e2e/...
    golangci-lint run --max-same-issues 0 --max-issues-per-linter 0 --build-tags e2e ./internal/...
    golangci-lint run --max-same-issues 0 --max-issues-per-linter 0 --build-tags realhosts ./test/realhosts/...
