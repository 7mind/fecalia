...
// so its byte-rate headroom is too small to build a 1200ms queue
...
func TestProbeHeadroomUnderOverload(t *testing.T) {
	...
	delta := func(name string) float64 {
		...
		if !ok {
			t.Fatalf("after scrape missing %s{path=%q}", name, t145Path.name)
		}
		b, ok := before.PathValue(name, t145Path.name)
		if !ok {
			t.Fatalf("before scrape missing %s{path=%q}", name, t145Path.name)
		}
		return a - b
	}
