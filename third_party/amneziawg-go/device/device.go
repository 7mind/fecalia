...
func NewDevice(tunDevice tun.Device, bind conn.Bind, logger *Logger) *Device {
	device := new(Device)
	device.protocol = defaultProtocolState()
...
func (device *Device) resetProtocol() {
	...
	defer device.awg.ASecMux.Unlock()
	device.protocol = defaultProtocolState()
	device.cookieChecker.SetMessageCookieReplyType(device.protocol.messageCookieReplyType)
}
...
func (device *Device) handlePostConfig(tempAwg *awg.Protocol) error {
	...
	if tempAwg.ASecCfg.InitPacketMagicHeader > 4 {
		...
		device.protocol.messageInitiationType = device.awg.ASecCfg.InitPacketMagicHeader
	} else {
		device.log.Verbosef("UAPI: Using default init type")
		device.protocol.messageInitiationType = DefaultMessageInitiationType
	}
	...
	if tempAwg.ASecCfg.ResponsePacketMagicHeader > 4 {
		...
		device.protocol.messageResponseType = device.awg.ASecCfg.ResponsePacketMagicHeader
	} else {
		device.log.Verbosef("UAPI: Using default response type")
		device.protocol.messageResponseType = DefaultMessageResponseType
	}
	...
	if tempAwg.ASecCfg.UnderloadPacketMagicHeader > 4 {
		...
		device.protocol.messageCookieReplyType = device.awg.ASecCfg.UnderloadPacketMagicHeader
	} else {
		device.log.Verbosef("UAPI: Using default underload type")
		device.protocol.messageCookieReplyType = DefaultMessageCookieReplyType
	}
	device.cookieChecker.SetMessageCookieReplyType(device.protocol.messageCookieReplyType)
	...
	if tempAwg.ASecCfg.TransportPacketMagicHeader > 4 {
		...
		device.protocol.messageTransportType = device.awg.ASecCfg.TransportPacketMagicHeader
	} else {
		device.log.Verbosef("UAPI: Using default transport type")
		device.protocol.messageTransportType = DefaultMessageTransportType
	}

	isSameMap := map[uint32]struct{}{
		device.protocol.messageInitiationType:  {},
		device.protocol.messageResponseType:    {},
		device.protocol.messageCookieReplyType: {},
		device.protocol.messageTransportType:   {},
	}
	...
	if newInitSize == newResponseSize {
		...
		device.protocol.packetSizeToMsgType = map[int]uint32{
			newInitSize:            device.protocol.messageInitiationType,
			newResponseSize:        device.protocol.messageResponseType,
			MessageCookieReplySize: device.protocol.messageCookieReplyType,
			MessageTransportSize:   device.protocol.messageTransportType,
		}

		device.protocol.msgTypeToJunkSize = map[uint32]int{
			device.protocol.messageInitiationType:  device.awg.ASecCfg.InitPacketJunkSize,
			device.protocol.messageResponseType:    device.awg.ASecCfg.ResponsePacketJunkSize,
			device.protocol.messageCookieReplyType: 0,
			device.protocol.messageTransportType:   0,
		}
	}
