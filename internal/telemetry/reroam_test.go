...
func TestReflectorReRoamSameSessionNoEpochReset(t *testing.T) {
	...
	if _, err := r.Reflect(encodeProbe(t, psk, pathID, sess, next, false, 0)); err != nil {
	...
	if _, err := r.Reflect(encodeProbe(t, psk, pathID, sess, preRoamHighWater, false, 0)); !errors.Is(err, ErrReplay) {
	...
	echo, err := r.Reflect(encodeProbe(t, psk, pathID, 0x9999_9999_9999_9999, 0, false, 0))
	if err != nil {
		t.Fatalf("restart bootstrap after re-roam rejected: %v", err)
	}
	ch := echoChallenge(t, psk, echo)
	if _, err := r.Reflect(encodeProbe(t, psk, pathID, 0x9999_9999_9999_9999, 1, false, ch)); err != nil {
